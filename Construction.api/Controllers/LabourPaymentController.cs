...
namespace Construction.api.Controllers
{
    ...
    public class LabourPaymentController : ControllerBase
    {
        ...
        [HttpGet("GetLabourData")]
        public async Task<IActionResult> GetLabourData()
        {
            var orgIdClaim = User.FindFirst("OrganisationId")?.Value;
            if (string.IsNullOrEmpty(orgIdClaim) || !Guid.TryParse(orgIdClaim, out var organisationId))
                return Unauthorized(new { message = "OrganisationId claim missing or invalid in token." });

            var result = await _service.GetLabourEmployeesAndSitesAsync(organisationId);
            return Ok(result);
        }

        [HttpPost("Add")]
        public async Task<IActionResult> Add([FromBody] LabourPaymentRequestModel dto)
        {
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] LabourPaymentRequestModel dto)
        {
            if (dto == null || dto.LabourPaymentId == Guid.Empty) return BadRequest("Invalid payload");
